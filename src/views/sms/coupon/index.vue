      ...
      <div style="margin-top: 15px">
        <el-form :inline="true" :model="listQuery" size="small" label-width="140px">
          <el-form-item label="优惠券名称：">
            <el-input v-model.trim="listQuery.name" class="input-width" placeholder="优惠券名称"></el-input>
          </el-form-item>
        </el-form>
      </div>
    </el-card>
  ...
  export default {
    ...
    methods:{
      ...
      handleView(index, row) {
        this.$router.push({path: '/sms/couponHistory', query: {id: row._id}})
      },
      handleUpdate(index, row) {
        this.$router.push({path: '/sms/updateCoupon', query: {id: row._id}})
      },
      handleDelete(index, row) {
        this.$confirm('是否进行删除操作?', '提示', {
          ...
          this.$axios.get('/coupon/remove',{params:{_id:row._id}}).then(res=>{
      ...
      getList(){
        this.$axios.get('/coupon/all',{params:this.listQuery}).then(res=>{
          if (res.status === 200 && res.data.err_code === 0){
