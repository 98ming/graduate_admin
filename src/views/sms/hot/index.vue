      ...
      <div style="margin-top: 15px">
        <el-form :inline="true" :model="listQuery" size="small" label-width="140px">
          ...
          <el-form-item label="推荐状态：">
            <el-select v-model="listQuery.recommandStatus" placeholder="全部" clearable class="input-width">
    ...
    <div class="table-container">
      <el-table ref="newProductTable"
        ...
        <el-table-column label="状态" width="160" align="center">
          <template slot-scope="scope">{{scope.row.recommandStatus | formatRecommendStatus}}</template>
        </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      handleRecommendStatusStatusChange(index,row){
        this.updateRecommendStatusStatus(row._id,row.recommandStatus);
      },
      ...
      handleBatchOperate(){
        ...
        if (this.operateType === 0) {
          ...
          ids.forEach((item,index)=>{
            this.$axios.post('/productDetail/update',{_id:item,recommandStatus:1}).then(res=>{
          ...
          //取消推荐
          ids.forEach((item,index)=>{
            this.$axios.post('/productDetail/update',{_id:item,recommandStatus:0}).then(res=>{
              if (res.status === 200 && res.data.err_code === 0){
                if (index === ids.length - 1){
                  this.getList();
                  this.$message({
      ...
      updateRecommendStatusStatus(ids,status){
        this.$confirm('是否要修改推荐状态?', '提示', {
          ...
          this.$axios.post('/productDetail/update',{_id:ids,recommandStatus:status}).then(res=>{
            if (res.status === 200 && res.data.err_code === 0){
              this.$message({
                message:'此商品信息更新成功',
                type:'success'
