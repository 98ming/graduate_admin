    ...
    methods: {
      ...
      handleConfirm(){
        if(this.operateReasonId==null){
          //添加操作
          this.$axios.post('/returnReason/add',this.returnReason).then(res=>{
          ...
          //编辑操作
          this.$axios.post('/returnReason/update',{_id:this.operateReasonId,dataObj:this.returnReason}).then(res=>{
            if (res.status === 200 && res.data.err_code === 0){
              this.$message({
                type:'success',
                message:res.data.message
      ...
      getList(){
        this.$axios.get('/returnReason/all',{params:this.listQuery}).then(res=>{
      ...
      deleteReason(ids){
        this.$confirm('是否要进行该删除操作?', '提示', {
          ...
          ids.forEach((item,index)=>{
            this.$axios.get('/returnReason/remove',{params:{_id:item}}).then(res=>{
              if (res.status === 200 && res.data.err_code === 0){
